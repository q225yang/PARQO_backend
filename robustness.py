...
parser.add_argument('--rqo_query', type=str, required=False, default=None, help='The path to the query that you want to calculate RQO')
...
def get_plan_list(sensitive_rels):
    ...
    if args.inst_id != None:
        ...
        if db_name == 'imdbloadbase': # use the plan generated by full err profile (no train-test split)
            if RANK_BY_PROB:
                file = f'./plan/{ON_SAMPLE}pqo-entire-space/rank-by-prob/on-basetmp_plan_dict_{db_name}_{rqo_id}.txt'
            # else:
            #     file = f'./plan/{ON_SAMPLE}pqo-entire-space/on-basetmp_plan_dict_{db_name}_{get_pure_q_id(query_id, db_name)}-{i}{cal_sen_method}-b{args.b}.txt'
...
def exp_penalty_by_samples(cur_plan_list, sensitive_rels, joint_error_samples, tolerance, est_base_sel=est_base_sel, est_join_sel=est_join_sel, save_samples=True):
    ...
    if save_samples and args.method == 'sobol':
        ...
        if RANK_BY_PROB:
            filename = f'reuse/rank-by-prob/{rqo_id}.json'
        else:
            filename = f'reuse/{db_name}_{query_id}.json'
...
def rqo():
    if RANK_BY_PROB:
        logging.basicConfig(filename='./log/' + ON_SAMPLE + 'rank-by-prob/' + db_name + '_' + rqo_id + '_' + str(tolerance) + cal_sen_method + '.log', level=logging.INFO)
    else:
    ...
    if not args.cal_sen:
        ...
        if RANK_BY_PROB:
            sensitive_rels = sorted(sen_dict[rqo_id]) # read from file
        else:
...
def pqo_query_subset(N=100):
    ...
    sql_template, literal_list = gen_sql_by_template(query_id, N, db_name, rank_by_prob=RANK_BY_PROB)
    ...
    for q_id, optimized_query in enumerate(optimized_queries_dict.values()):
        if RANK_BY_PROB:
            ...
            filename = f'query/join-order-benchmark/pqo/{get_pure_q_id(query_id, db_name)}-{q_id}-b{args.b}.sql'
...
def pqo(N=100):
    ...
    cursor = conn.cursor()
    if RANK_BY_PROB:
        filename = './log/' + ON_SAMPLE + 'rank-by-prob/pqo/' + db_name + '_' + query_id + '_pqo_' + str(tolerance) + cal_sen_method + '.log'
    else:
    ...
    for sql_id, para_sql in enumerate(sql_template):
        ...
        if os.path.exists(filename):
            ...
            for i in range(len(optimized_queries_dict.values())):
                ...
                if RANK_BY_PROB:
                    ...
                    plan_file = f'./plan/{ON_SAMPLE}pqo-entire-space/saved-tmp_plan_dict_{db_name}_{get_pure_q_id(query_id, db_name)}-{i}{cal_sen_method}-b{args.b}.txt'
            ...
            if RANK_BY_PROB:
                print(f"{get_pure_q_id(query_id, db_name)}-{nearest_query_id}-b{args.b}")
                plan_space = cached_rob_plan_dict_by_prob[f"{get_pure_q_id(query_id, db_name)}-{nearest_query_id}-b{args.b}"]
            else:
        ...
        if RANK_BY_PROB:
            ...
            filename = f'reuse/{db_name}/pqo-{query_id}-b{args.b}.txt'
